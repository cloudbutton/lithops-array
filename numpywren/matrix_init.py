...
from . import matrix
from .matrix_utils import generate_key_name_local_matrix, MmapArray
...
def mmap_put_block(bigm, mmap_array, bidxs_blocks):
    ...
    slices = tuple([slice(s, e) for s, e in blocks])
...
def shard_matrix(bigm, X_local, n_jobs=1, overwrite=True):
    ...
    X_local_mmaped = np.memmap("{0}/{1}".format(npw.TMP_DIR, bigm.key),
                               dtype=bigm.dtype, shape=bigm.shape, mode="w+")
    ...
    futures = []
    with fs.ThreadPoolExecutor(n_jobs) as ex:
        for (bidxs, blocks) in zip(all_bidxs, all_blocks):
            future = ex.submit(mmap_put_block, bigm, X_local_mmap, zip(bidxs, blocks))
...
def reshard_down(bigm, breakdowns, fexec=None):
    ''' Return a new bigm whose shard sizes are bigm.shard_sizes/break_downs
        ...
        sized sub blocks, breakdowns is a list of integers detailing how much to break
        a given dimension down into. breakdowns = [2,2] would break each dimension down
        into 2 independent block so a 4 x 4 block would be replaced by 4, 2 x 2 blocks.
    '''
    ...
    X_sharded_new = BigMatrix("reshard({0},{1})".format(bigm.key, breakdowns),
                              bucket=bigm.bucket, shape=bigm.shape, shard_sizes=new_shard_sizes)
    ...
    if (fexec is None):
        ...
        futures = fexec.map(lambda x: reshard_func(x, bigm, X_sharded_new), idx_info)
        fexec.wait(futures)
        [f.result() for f in futures]
