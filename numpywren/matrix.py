...
from .matrix_utils import list_all_keys, block_key_to_block, key_exists
from . import utils
...
DEFAULT_BUCKET = 'numpy-lithops'
DEFAULT_REGION = ''


class BigMatrix:
    """
    A multidimensional array stored in an storage service, sharded in blocks of a given size.
    ...
    def __init__(self,
        ...
        if shape is None or shard_sizes is None:
    ...
    def submatrix(self, *block_slices):
        ...
        for _, block_slice in enumerate(block_slices):
    ...
    def blocks_exist(self):
        ...
        blocks : array_like of int
            ...
            dimension.
        """
        self._create_storage_client()
        all_keys = list_all_keys(self.bucket, self.key_base, self.storage)
    ...
    def get_block(self, *block_idx):
        ...
        """
        print('BigMatrix: {} - Get block: {}'.format(self.key, block_idx))
        self._create_storage_client()
        ...
        exists = key_exists(self.bucket, key, self.storage)

        if not exists and dill.loads(self.parent_fn) is None:
        ...
        elif not exists and dill.loads(self.parent_fn) is not None:
    ...
    def put_block(self, block, *block_idx, no_overwrite=False):
        """
        ...
        """
        print('BigMatrix: {} - Put block: {}'.format(self.key, block_idx))
        self._create_storage_client()

        key = self.__shard_idx_to_key__(block_idx)
        if no_overwrite:
            if key_exists(self.bucket, key, self.storage):
                old_block = self.get_block_async(*block_idx)
    ...
    def delete_block(self, *block_idx):
        ...
        """
        print('BigMatrix: {} - Delete block: {}'.format(self.key, block_idx))
        key = self.__shard_idx_to_key__(block_idx)
        self._create_storage_client()
        resp = self.storage.delete_object(bucket=self.bucket, key=key)
    ...
    def __read_header__(self):
        ...
        try:
            ...
            header = json.loads(self.storage.get_object(bucket=self.bucket, key=key)['Body'])
    ...
    def __delete_header__(self):
        key = os.path.join(self.key_base, "header")
        self._create_storage_client()
        self.storage.delete_object(bucket=self.bucket, key=key)
    ...
    def __s3_key_to_byte_io__(self, key):
        ...
        while bio is None and n_tries <= max_n_tries:
            try:
                matrix_bytes = self.storage.get_object(bucket=self.bucket, key=key)
                bio = io.BytesIO(matrix_bytes)
            except Exception:
                n_tries += 1
        if bio is None:
            raise Exception("Storage Read Failed")
        return bio
    ...
    def __save_matrix_to_s3__(self, X, out_key):
        ...
        outb = io.BytesIO()
        np.save(outb, X)
        self.storage.put_object(key=out_key, bucket=self.bucket, body=outb.getvalue())
    ...
    def __write_header__(self):
        ...
        self.storage.put_object(key=key,
                                bucket=self.bucket,
