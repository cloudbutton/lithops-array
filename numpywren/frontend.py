...
#from numpywren.compiler import lpcompile
...
class LambdaPackTypeCheck(ast.NodeVisitor):
    ...
    def visit_BinOp(self, node):
        ...
        if (op == "Or" or op == "And"):
            ...
            if ((r_type is None) or (l_type is None)):
                raise exceptions.LambdaPackTypeException("BinOp arguments must be typed")
            type_set = set([r_type, l_type])
            for t in type_set:
                if (not issubclass(t, NumericalType)):
                    raise exceptions.LambdaPackTypeException("BinOp arguments must be Numerical")
            if (op == "Add" or op == "Sub"):
                # arith type algebra
    ...
    def visit_For(self, node):
        ...
        if ((not linear_max) or (not linear_min)):
            raise exceptions.LambdaPackTypeException("Loop bounds must be LinearIntType")
    ...
    def visit_Reduction(self, node):
        ...
        for expr_i in expr:
            if (not isinstance(expr_i, IndexExpr)):
                raise exceptions.LambdaPackTypeException("Reduction Exprs must be of IndexExpr type")
    ...
    def visit_FuncDef(self, node):
        ...
        for i, (arg, arg_value, arg_type)  in enumerate(zip(node.args, self.arg_values, node.arg_types)):
            ...
            if (not issubclass(p_type, arg_type)):
                raise exceptions.LambdaPackBackendGenerationException("arg {0} wrong type expected {1} got {2}".format(i, arg_type, p_type))
