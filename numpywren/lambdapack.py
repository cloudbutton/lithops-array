...
from .matrix_utils import chunk
...
class RemoteRead(RemoteInstruction):
    def __init__(self, i_id, matrix, *bidxs):
        ...
        self.read_size = np.product(
            self.matrix.shard_sizes)*np.dtype(self.matrix.dtype).itemsize
    ...
    async def __call__(self):
        ...
        if (self.result is None):
            cache_key = (self.matrix.key, self.matrix.bucket,
                         self.matrix.true_block_idx(*self.bidxs))
    ...
    async def __call__(self, prev=None):
        ...
        def compute():
            ...
            if (isinstance(results, tuple) and len(results) != len(self.results)):
                raise Exception("Expected {0} results, got {1}".format(
...
class LambdaPackProgram(object):
    ...
    def __init__(self, program, config, num_priorities=1, eager=False, block_sparse=False):
        ...
        for i in range(num_priorities):
            queue_url = client.create_queue(
                QueueName=self.hash + str(i))["QueueUrl"]
    ...
    def _node_str(self, expr_idx, var_values):
        var_strs = sorted(["{0}:{1}".format(key, value)
                           for key, value in var_values.items()])
        return "{0}_({1})".format(expr_idx, "-".join(var_strs))

    def get_node_status(self, expr_idx, var_values):
        s = get(self.control_plane.client,
                self._node_key(expr_idx, var_values))
        if (s == None):
            s = 0
        return NS(int(s))

    def set_node_status(self, expr_id, var_values, status):
        put(self.control_plane.client, self._node_key(
            expr_id, var_values), status.value)
        return status
        ...
        try:
            ...
            if (ret_code == PS.EXCEPTION and tb != None):
                self.handle_exception(
                    " EXCEPTION", tb=tb, expr_idx=expr_idx, var_values=var_values)
            ready_children = []
            ...
            for child in children:
                ...
                val_future = tp.submit(
                    conditional_increment, self.control_plane.client, child_edge_sum_key, my_child_edge)
            ...
            for child in ready_children:
                ...
                message_body = json.dumps(
                    [int(child[0]), {str(key): int(val) for key, val in child[1].items()}])
                resp = client.send_message(
                    QueueUrl=self.queue_urls[0], MessageBody=message_body)
            ...
            self.incr_progress()
            profiling_info = self.dump_profiling_info(
                inst_block, expr_idx, var_values)
            e = time.time()
            ...
            if (terminator):
                ...
                return_edge = self._edge_key(
                    expr_idx, var_values, return_key, {})
                tp = fs.ThreadPoolExecutor(1)
                val_future = tp.submit(
                    conditional_increment, self.control_plane.client, return_key, return_edge)
                done, not_done = fs.wait([val_future], timeout=60)
                if len(done) == 0:
                    raise Exception("Redis Atomic Set and Sum timed out!")
                val = val_future.result()
                print("Num finished terminators", val,
                      "num terminators total", self.program.num_terminators)
            ...
            traceback.print_exc()
            self.handle_exception("POST OP EXCEPTION", tb=tb,
                                  expr_idx=expr_idx, var_values=var_values)
            raise
        ...
        def start_chunk(c):
            ...
            for x in c:
                ...
                queue.send_message(MessageBody=json.dumps(
                    [x[0], {str(key): val for key, val in x[1].items()}]))
        if (parallel):
            fexec = lithops.FunctionExecutor()
            futures = fexec.map(start_chunk, chunked_starters)
            fexec.wait(futures)
        else:
    ...
    def stop(self):
        client = boto3.client('s3')
        client.put_object(Key="lambdapack/" + self.hash + "/EXCEPTION.DRIVER.CANCELLED",
                          Bucket=self.bucket, Body="cancelled by driver")
    ...
    def handle_exception(self, error, tb, expr_idx, var_values):
        client = boto3.client('s3')
        client.put_object(Key="lambdapack/" + self.hash + "/EXCEPTION.{0}".format(
            self._node_str(expr_idx, var_values)), Bucket=self.bucket, Body=tb + str(error))
        e = PS.EXCEPTION.value
        put(self.control_plane.client, self.hash, e)
    ...
    def incr_repeated_compute(self, amount=1):
        incr(self.control_plane.client,
             "{0}_repeated_compute".format(self.hash), amount)

    def incr_repeated_post_op(self, amount=1):
        incr(self.control_plane.client,
             "{0}_repeated_post_op".format(self.hash), amount)

    def incr_repeated_finish(self, amount=1):
        incr(self.control_plane.client,
             "{0}_repeated_finish".format(self.hash), amount)

    def incr_not_ready(self, amount=1):
        incr(self.control_plane.client,
             "{0}_not_ready".format(self.hash), amount)
    ...
    def incr_flops(self, amount):
        if (amount > 0):
            incr(self.control_plane.client,
                 "{0}_flops".format(self.hash), amount)

    def incr_read(self, amount):
        if (amount > 0):
            incr(self.control_plane.client,
                 "{0}_read".format(self.hash), amount)

    def incr_sparse_read(self, amount):
        if (amount > 0):
            incr(self.control_plane.client,
                 "{0}_sparse_read".format(self.hash), amount)

    def incr_write(self, amount):
        if (amount > 0):
            incr(self.control_plane.client,
                 "{0}_write".format(self.hash), amount)

    def incr_sparse_write(self, amount):
        if (amount > 0):
            incr(self.control_plane.client,
                 "{0}_write_sparse".format(self.hash), amount)

    def decr_flops(self, amount):
        if (amount > 0):
            decr(self.control_plane.client,
                 "{0}_flops".format(self.hash), amount)
    ...
    def get_profiling_info(self, expr_idx, var_values):
        try:
            client = boto3.client('s3')
            byte_string = client.get_object(
                Bucket=self.bucket, Key="lambdapack/{0}/{1}_{2}".format(self.hash, expr_idx, var_values))["Body"].read()
