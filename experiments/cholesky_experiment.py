...
def run_experiment(problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores, start_cores, trial, launch_granularity, timeout, log_granularity, autoscale_policy, standalone, warmup, verify, matrix_exists, read_limit, write_limit, n_threads):
    ...
    if standalone:
        ...
        pwex = pywren.standalone_executor(
            config=config, job_max_runtime=999999)
    else:
    ...
    if (not matrix_exists):
        ...
        X_sharded = BigMatrix("sosp_cholesky_test_{0}_{1}".format(problem_size, shard_size), shape=X.shape,
                              shard_sizes=shard_sizes, write_header=True, autosqueeze=False, bucket="numpywrentest", parent_fn=constant_zeros)
    ...
    try:
        while(program.program_status() == lp.PS.RUNNING):
            ...
            sparse_writes = parse_int(REDIS_CLIENT.get(
                "{0}_write_sparse".format(program.hash)))/1e9
            ...
            print("{2}: Up Workers: {0}, Busy Workers: {1}".format(
                up_workers, busy_workers, curr_time))
            print("{0}: Total GFLOPS {1}, Total GBytes Read {2}, Total GBytes Write {3}, Total Gbytes Write Sparse : {4}".format(
                curr_time, current_gflops, current_gbytes_read, current_gbytes_write, sparse_writes))
