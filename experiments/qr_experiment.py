...
def run_experiment(problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores, start_cores, trial, launch_granularity, timeout, log_granularity, autoscale_policy, standalone, warmup, verify, matrix_exists, read_limit, write_limit, compute_threads_per_worker):
    ...
    if standalone:
        extra_env = {"AWS_ACCESS_KEY_ID": "AKIAJO7J456T575BNHTA", "AWS_SECRET_ACCESS_KEY":
                     "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", "OMP_NUM_THREADS": "1", "AWS_DEFAULT_REGION": region}
        config = wc.default()
    ...
    if (not matrix_exists):
        ...
        X_sharded = BigMatrix("qr_test_{0}_{1}".format(problem_size, shard_size), shape=X.shape,
                              shard_sizes=shard_sizes, write_header=True, autosqueeze=False, bucket="numpywrennsdi2")
        ...
        X_sharded = BigMatrix("qr_test_{0}_{1}".format(
            problem_size, shard_size), autosqueeze=False, bucket="numpywrennsdi2")
