...
def BDFAC(VLs: BigMatrix, TLs: BigMatrix, Rs: BigMatrix, Sigma: BigMatrix, VRs: BigMatrix, TRs: BigMatrix, S0: BigMatrix, S1: BigMatrix, N: int, truncate: int):
    ...
    for i in range(0, N):
        ...
        for j in range(i, N):
            VLs[j, i, N_tree], TLs[j, i, N_tree], Rs[j,
                                                     i, N_tree] = qr_factor(S1[j, i, i, 0])

        with reducer(expr=Rs[j, i, N_tree], var=j, start=i, end=N, b_fac=b_fac) as r:
            VLs[j, i, N_tree - r.level - 1], TLs[j, i, N_tree - r.level -
                                                 1], Rs[j, i, N_tree - r.level - 1] = qr_factor(*r.reduce_args)
            r.reduce_next(Rs[j, i, N_tree - r.level - 1])

        # flat trailing matrix update
        for j in range(i, N):
            for k in range(i+1, N):
                S0[j, k, i+1, N_tree] = qr_leaf(VLs[j, i, N_tree],
                                                TLs[j, i, N_tree], S1[j, k, i, 0])

        for k in range(i+1, N):
            with reducer(expr=S0[j, k, i+1, N_tree], var=j, start=i, end=N, b_fac=b_fac) as r:
                S0[j, k, i+1, N_tree - r.level - 1], S0[j + b_fac**r.level, k, i+1, 0] = qr_trailing_update(
                    VLs[j, i, N_tree - r.level - 1], TLs[j, i, N_tree - r.level - 1], *r.reduce_args)
        ...
        for j in range(i, N):
            VRs[j, i, N_tree], TRs[j, i, N_tree], Sigma[j,
                                                        i, N_tree] = lq_factor(S0[j, i, i, 0])

        with reducer(expr=Sigma[j, i, N_tree], var=j, start=i, end=N, b_fac=b_fac) as r:
            VRs[j, i, N_tree - r.level - 1], TRs[j, i, N_tree - r.level -
                                                 1], Sigma[j, i, N_tree - r.level - 1] = lq_factor(*r.reduce_args)
            r.reduce_next(Sigma[j, i, N_tree - r.level - 1])

        for j in range(i, N):
            for k in range(i+1, N):
                S1[j, k, i+1, N_tree] = lq_leaf(VRs[j, i, N_tree],
                                                TRs[j, i, N_tree], S0[j, k, i, 0])

        for k in range(i+1, N):
            with reducer(expr=S1[j, k, i+1, N_tree], var=j, start=i, end=N, b_fac=b_fac) as r:
                S1[j, k, i+1, N_tree - r.level - 1], S1[j + b_fac**r.level, k, i+1, 0] = lq_trailing_update(
                    VRs[j, i, N_tree - r.level - 1], TRs[j, i, N_tree - r.level - 1], *r.reduce_args)
