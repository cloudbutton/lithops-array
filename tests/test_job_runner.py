...
def test_cholesky():
    ...
    A_sharded = BigMatrix("job_runner_test", shape=A.shape,
...
def test_cholesky_multiprocess():
    ...
    for i in range(8):
        future = executor.submit(
            job_runner.lambdapack_run, program, timeout=25)
...
def test_cholesky_lambda():
    ...
    futures = pwex.map(lambda x: job_runner.lambdapack_run(
...
def test_cholesky_multi_repeats():
    ...
    for c in range(cores):
        p = mp.Process(target=job_runner.lambdapack_run, args=(
            program,), kwargs={'timeout': 3600, 'pipeline_width': 3})
        jobs.append(p)
        p.start()
